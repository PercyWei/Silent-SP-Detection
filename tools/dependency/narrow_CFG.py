# This code is copied from https://github.com/duo-labs/narrow
# Original file: cfg.py
# Original author: duo-labs
...
class ControlFlowGraph:
    ...
    def _parse_and_resolve_tree_sitter(self, init_ast_chunk, init_context: typing.List[str],
        ...
        while not tasks.empty():
            ...
            if ast_chunk.type == 'module':
                ...
                tasks.put(
                    ControlFlowGraph.ResolveTask(ast_chunk.child_by_field_name('body'), context, current_file_location))
                ...
                if func_name:
                    if not self.function_exists(func_name, arg_count):
                        ...
                        for (call_def, call_file) in call_defs:
                            ...
                            if (call_def and
                                    call_def.child_by_field_name('name').text.decode(
                                        'utf-8') in self._functions_to_locate):
                                self._detected = True
                                return
                ...
                for child in ast_chunk.children:
                    ...
                    if value is not None:
                        tasks.put(ControlFlowGraph.ResolveTask(child.child_by_field_name('value'), context,
                                                               current_file_location))
    ...
    def _find_function_def_nodes_matching_name_tree_sitter(self, func_name: str,
                                                           ...
                                                           arg_count: int,
                                                           class_type: str = 'unknown') -> typing.List[
        typing.Tuple[any, str]]:
    ...
    def _find_entries_for_import(self, import_name: str,
        ...
        if (module != '' and module + '.' + import_name in self._imports and self._imports[module + '.' + import_name][
            'path']):
        ...
        if module != '' and len(results) == 0:
            ...
            for root, dirs, _ in os.walk(
                    self.get_folder_back_n_dirs(pathlib.Path(current_file_location), self.module_backtrace_max)):
                for dir in dirs:
                    if dir == module:
