...
def combine_locations_before_and_after(comb_info: CombineInfo, file_diff_lines: List[DiffLine]) -> CombineInfo:
    ...
    def _get_start_unchanged_line_id(lo: Location, del_or_add_lines: List[int]) -> int | None:
        range_ids = lo.get_full_range()
        for li in range_ids:
            if li not in del_or_add_lines:
                return li
        return None

    def _is_same_loc(old_lo: Location, new_lo: Location) -> bool:
        old_start_li = _get_start_unchanged_line_id(old_lo, del_line_ids)
        new_start_li = _get_start_unchanged_line_id(new_lo, add_line_ids)
        ...
        while i < len(old_children) or j < len(new_children):
            ...
            if new_child is None:
                ...
                comb_lo = CombLocation(id=curr_id, father=father_id, type=old_child.type,
                                       ast=old_child.ast, name="", range=comb_range,
                ...
                comb_lo = CombLocation(id=curr_id, father=father_id, type=new_child.type,
                                       ast=new_child.ast, name="", range=comb_range,
    ...
    old_root_loc = old_locations.pop(0)
    new_root_loc = new_locations.pop(0)

    assert old_root_loc.type == LocationType.MODULE and new_root_loc.type == LocationType.MODULE

    start = min(old_li_lookup[old_root_loc.range.start], new_li_lookup[new_root_loc.range.start])
    end = max(old_li_lookup[old_root_loc.range.end], new_li_lookup[new_root_loc.range.end])

    curr_id = _get_curr_loc_id()
    comb_root_loc = CombLocation(id=curr_id, father=None, type=LocationType.MODULE,
                                 ast=old_root_loc.ast, name="", range=LineRange(start, end),
                                 name_before=old_root_loc.name, name_after=new_root_loc.name)
    comb_locations[curr_id] = comb_root_loc

    ##################################################
    # Step 2: Collect children of MODULE and combine #
    ##################################################

    old_root_children, old_locations = _collect_children(old_locations, old_root_loc.id)
    new_root_children, new_locations = _collect_children(new_locations, new_root_loc.id)

    _combine_children(old_root_children, new_root_children, comb_root_loc.id)
    ...
    # (1) Get MAIN location
    comb_main_loc_id: int | None = None
    for _, comb_loc in comb_locations.items():
        if comb_loc.type == LocationType.MAIN:
            assert comb_main_loc_id is None
            comb_main_loc_id = comb_loc.id

    # (2) Combine old and new children of MAIN location
    if comb_main_loc_id is not None:
        old_main_loc_id = comb2old_loc_lookup[comb_main_loc_id]
        new_main_loc_id = comb2new_loc_lookup[comb_main_loc_id]

        old_main_children: List[Location] = []
        new_main_children: List[Location] = []

        if old_main_loc_id is not None:
            old_main_children, old_locations = _collect_children(old_locations, old_main_loc_id)

        if new_main_loc_id is not None:
            new_main_children, new_locations = _collect_children(new_locations, new_main_loc_id)

        _combine_children(old_main_children, new_main_children, comb_main_loc_id)
    ...
    comb_class_loc_ids: List[int] = []
    for _, comb_loc in comb_locations.items():
        if comb_loc.type == LocationType.CLASS:
            comb_class_loc_ids.append(comb_loc.id)

    # (2) Combine old and new children of each CLASS location
    for comb_class_loc_id in comb_class_loc_ids:
        old_class_loc_id = comb2old_loc_lookup[comb_class_loc_id]
        new_class_loc_id = comb2new_loc_lookup[comb_class_loc_id]

        old_class_children: List[Location] = []
        new_class_children: List[Location] = []

        if old_class_loc_id is not None:
            old_class_children, old_locations = _collect_children(old_locations, old_class_loc_id)

        if new_class_loc_id is not None:
            new_class_children, new_locations = _collect_children(new_locations, new_class_loc_id)

        _combine_children(old_class_children, new_class_children, comb_class_loc_id)
...
def _match_diff_main(
        old_file_lines: List[str], old_locations: Dict[int, Location], old_diff_line_ids: List[int], old_main: int | None,
        new_file_lines: List[str], new_locations: Dict[int, Location], new_diff_line_ids: List[int], new_main: int | None
) -> List[LocationPair]:
    """
    ...
    """
    if old_main is None and new_main is None:
        return []

    if old_main is not None and new_main is not None:
        old_main_loc = old_locations[old_main]
        new_main_loc = new_locations[new_main]

        assert _is_modified_struct(
            old_file_lines, old_diff_line_ids, old_main_loc,
            new_file_lines, new_diff_line_ids, new_main_loc
        )

    return [LocationPair(before=old_main, after=new_main)]
...
def match_diff_structs_within_file(
    ...
    diff_ifMain_pairs = _match_diff_main(
        old_file_lines, old_locations, del_line_ids, old_structs_info["main"],
        new_file_lines, new_locations, add_line_ids, new_structs_info["main"]
    )
...
def group_diff_lines_by_struct_within_file(
    ...
    for diff_line in file_diff_lines:
        ...
        if last_line_diffloc is None:
            ...
            if last_line_diffloc.type in class_child_loc_types and cur_line_diffloc.type in class_child_loc_types:
                ...
                new_group_flag = True
            elif last_line_diffloc.type == LocationType.MAIN_UNIT:
    ...
    def _split_diff_lines_in_global(_global_line_group: DiffLineGroup) -> List[DiffLineGroup]:
        ...
        """
        assert _global_line_group.loc_type in (LocationType.UNIT, LocationType.MAIN_UNIT, LocationType.CLASS_UNIT)
    ...
    def _update_class_diff_line_group(_class_line_group: DiffLineGroup) -> DiffLineGroup:
        ...
        for _child in _class_line_group.children:
            if _child.loc_type == LocationType.CLASS_UNIT:
    ...
    for line_group in diff_line_groups:
        ...
        if line_group.loc_type == LocationType.UNIT:
...
def build_code_snippet_location_for_diff_line_group(
        file_path: str, diff_line_group: DiffLineGroup,
        old_locations: Dict[int, Location], old_line_loc_lookup: Dict[int, int],
        new_locations: Dict[int, Location], new_line_loc_lookup: Dict[int, int]
):
    if diff_line_group.loc_type == LocationType.MAIN:
        code_list = []
        for child_line_group in diff_line_group.children:
            child_csl = build_code_snippet_location_for_diff_line_group(
                file_path, child_line_group, old_locations, old_line_loc_lookup, new_locations, new_line_loc_lookup)
            code_list.append(child_csl.code)

        code = "\n...\n".join(code_list)

        return CodeSnippetLocation(file_path=file_path, class_name=None, func_name="__main__", code=code)
        ...
        return CodeSnippetLocation(file_path=file_path, class_name=None, func_name=func_name, code=code)
        ...
        return CodeSnippetLocation(file_path=file_path, class_name=None, func_name=None, code=code)
...
def build_diff_location_of_modified_file(
        old_locations: Dict[int, Location], old_line_loc_lookup: Dict[int, int],
        new_locations: Dict[int, Location], new_line_loc_lookup: Dict[int, int],
        file_diff_lines: List[DiffLine], all_diff_structs_info: AllDiffStructInfo
) -> List[CodeSnippetLocation]:
    """Build CodeSnippetLocations containing diff code snippet of modified file.
    ...
    ###########################################
    # Step II: Group diff lines by struct type #
    ###########################################
...
"""ALIGN STRUCTS IN COMBINED CODE"""


def _align_func_info_in_code_comb(
        comb_info: CombineInfo,
        all_old_funcs: List[int],
        diff_func_pairs: List[LocationPair],
) -> List[Tuple[str, LineRange]]:
    comb_funcs: List[Tuple[str, LineRange]] = []

    old_locations = comb_info.locs_before
    new_locations = comb_info.locs_after
    old_line_id_lookup = comb_info.li_lookup_before2comb
    new_line_id_lookup = comb_info.li_lookup_after2comb
    ...
    for diff_func_pair in diff_func_pairs:
        ...
        if new_func_loc_id is None:
            ...
            comb_range = LineRange(start=old_line_id_lookup[old_loc.range.start],
                                   end=old_line_id_lookup[old_loc.range.end])
            ...
            comb_range = LineRange(start=new_line_id_lookup[new_loc.range.start],
                                   end=new_line_id_lookup[new_loc.range.end])
            ...
            comb_start = min(old_line_id_lookup[old_start], new_line_id_lookup[new_start])
            comb_end = max(old_line_id_lookup[old_end], new_line_id_lookup[new_end])
    ...
    for func_loc_id in rest_old_funcs:
        ...
        comb_range = LineRange(start=old_line_id_lookup[func_loc.range.start],
                               end=old_line_id_lookup[func_loc.range.end])
...
def _align_class_info_in_code_comb(
        comb_info: CombineInfo,
        all_old_classes: List[int],
        diff_class_pairs: List[LocationPair]
) -> List[Tuple[str, LineRange]]:
    ...
    for class_loc_id in rest_old_classes:
        ...
        comb_range = LineRange(start=old_line_id_lookup[class_loc.range.start],
                               end=old_line_id_lookup[class_loc.range.end])
...
def _align_class_func_info_in_code_comb(
        comb_info: CombineInfo,
        all_old_classFuncs: Dict[int, List[int]],
        diff_classFuncs_pairs: Dict[LocationPair, List[LocationPair]],
    ...
    for diff_class_pair, cur_diff_classFunc_pairs in diff_classFuncs_pairs.items():
        if diff_class_pair.before is None:
            assert diff_class_pair.after is not None
            comb_classFuncs = _align_func_info_in_code_comb(comb_info, [], cur_diff_classFunc_pairs)
            ...
            assert diff_class_pair.before is not None
            comb_classFuncs = _align_func_info_in_code_comb(
                comb_info, all_old_classFuncs[diff_class_pair.before], cur_diff_classFunc_pairs
            )
    ...
    for class_loc_id in rest_old_classes:
        ...
        for old_classFunc in all_old_classFuncs[class_loc_id]:
            ...
            comb_classFunc_range = LineRange(start=old_line_id_lookup[old_classFunc_loc.range.start],
                                             end=old_line_id_lookup[old_classFunc_loc.range.end])
...
def parse_combine_content_structs(
        comb_info: CombineInfo, all_diff_structs_info: AllDiffStructInfo,
        all_old_funcs: List[int], all_old_classes: List[int], all_old_classFuncs: Dict[int, List[int]]
) -> Tuple[List[Tuple[str, LineRange]], List[Tuple[str, LineRange]], List[Tuple[str, List[Tuple[str, LineRange]]]]]:
    comb_funcs = _align_func_info_in_code_comb(comb_info, all_old_funcs, all_diff_structs_info.functions)

    comb_classes = _align_class_info_in_code_comb(comb_info, all_old_classes, all_diff_structs_info.classes)

    comb_classesFuncs = _align_class_func_info_in_code_comb(comb_info, all_old_classFuncs, all_diff_structs_info.class_functions)
...
def combine_and_parse_modified_file(
        old_code: str, old_locations: Dict[int, Location], old_structs_info: Dict,
        new_code: str, new_locations: Dict[int, Location],
        file_diff_lines: List[DiffLine],
        all_diff_structs_info: AllDiffStructInfo
) -> Tuple[CombineInfo, List[Tuple[str, LineRange]], List[Tuple[str, LineRange]], List[Tuple[str, List[Tuple[str, LineRange]]]]]:
    """Combine code before and after commit, and then analyze the structs of it.

    NOTE: Only for modified files.
    """
    ## Step 1: Preparation
    comb_info = CombineInfo(code_before=old_code, code_after=new_code,
                            locs_before=old_locations, locs_after=new_locations,
                            li2loc_before={}, li2loc_after={})

    ## Step 2: Combine old code and new code
    comb_info = combine_code_before_and_after(comb_info, file_diff_lines)

    ## Step 3: Align the diff structs
    comb_funcs, comb_classes, comb_classesFuncs = parse_combine_content_structs(
        comb_info, all_diff_structs_info,
        old_structs_info["funcs"], old_structs_info["classes"], old_structs_info["classes_funcs"]
    )

    return comb_info, comb_funcs, comb_classes, comb_classesFuncs
...
def analyse_modified_file(
        ...
        new_ori_code: str,
        diff_file_info: Dict
) -> Tuple[str, CombineInfo, List[Tuple[str, LineRange]], List[Tuple[str, LineRange]], List[Tuple[str, List[Tuple[str, LineRange]]]]]:
    ...
    ## (1) Filter code file
    old_nb_code, old_nb_li_lookup = filter_blank_lines_in_file(old_ori_code)
    new_nb_code, new_nb_li_lookup = filter_blank_lines_in_file(new_ori_code)
    ...
    old_nb_res = parse_python_file_locations(old_nb_code)
    new_nb_res = parse_python_file_locations(new_nb_code)

    # --------------------------- Step IV: Combine --------------------------- #
    old_nb_locs, old_nb_li2loc, _ = old_nb_res
    new_nb_locs, new_nb_li2loc, _ = new_nb_res
    ...
    #     nb_diff_lines, old_nb_code, old_nb_res, new_nb_code, new_nb_res
    # )
    #
    # ####################################################
    ...
    #     old_nb_locs, old_nb_li2loc, new_nb_locs, new_nb_li2loc, nb_diff_lines, all_diff_structs_info
    # )
    #
    # ############################################
    ...
    # ############################################
    #
    # comb_info, comb_funcs, comb_classes, comb_classesFuncs \
    #     = combine_and_parse_modified_file(
    #         old_nb_code, old_nb_locs, old_nb_structs,
    #         new_nb_code, new_nb_locs,
    #         nb_diff_lines, all_diff_structs_info
    #     )
    #
    #
    # return file_diff_desc, comb_info, comb_funcs, comb_classes, comb_classesFuncs
