...
def test(test_set, package, model, weights=None, cuda=0, seed=2022):
    ...
    cwe_tree_path = f"{DATA_PATH}/data/valid_cwe_tree.json"

    output_file = f"{MODEL_PATH}/test_results/{model}_metric.json"
    predictions_output_file = f"{MODEL_PATH}/test_results/{model}_result.json"
    
    _test(archive_file=f"{MODEL_PATH}/{model}/model.tar.gz", input_file=f"{DATA_PATH}/data/{test_set}.json", 
